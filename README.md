...
Event banners always wear **Open Sans** (any weight / italic in that family) plus emoji. Same energy as this page.
...
- 🎉 **Party** — the night closes on [drawing.grok.me](https://drawing.grok.me).
...
1. 📱 Join from a QR code, make a profile (18+), connect Google or email.
2. 💳 Pick how you show up: full event ($10 refundable deposit), party only ($10), monthly supporter ($50), or co-sponsor ($200).
3. 🚪 Check in at the door. A Steward scans your code or picks your name.
4. 📺 The room screen runs the cycle: check-in → meet → sponsors → lightning show-and-tell → build → review → vote → awards → party.
5. ⏱️ On your phone you see what’s now, what’s next, and the countdown. During the vote you drag presentations into order; the big screen mirrors it live.
...
Every event banner — on the site, on the paired screen, on phones, on print if we print it — follows the same two rules:

1. **Type is always a variation of [Open Sans](https://fonts.google.com/specimen/Open+Sans).** Regular, Medium, SemiBold, Bold, ExtraBold, Condensed, Italic — all fair. Not another family.
2. **Banners carry emoji.** Phase names on the big screen get the same marks you see in this README (🚪 Check-in, ⚡ Show and Tell, 🔨 Build, 🎉 Party, and so on) so the room and the repo speak one language.

Sponsor logos and videos sit *under* that type. They don’t replace the banner face.
...
1. The paired desktop shows the cycle. During downtime it shows the sponsor page. During a live phase it shows **what’s now** and a countdown to **what’s coming** — Open Sans + emoji on the banner.
...
| ⚖️ **Judge** | Presence, judgment, handing awards | Full participation plus the award moment | The sponsor’s taste, visible in the room. |
...
| 🛠️ **Admin** | Calendar, phases, approvals, refunds, pairing the screen | Dashboards, dual control, mobile transport | Keep the night honest and on the clock. |
...
- `packages/config` — methods, age gate, deposit, phase timings, party URL, **banner type = Open Sans family**
